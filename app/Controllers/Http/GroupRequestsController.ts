...
export default class GroupRequestsController {
  public async index ({ request, response}: HttpContextContract) {
    // O QS busca os parametros passados na url
    ...
    // preload carrega os relacionamentos, sem ele nãomtraz as informações do relacionamento na query
  ...
  public async store ({ request, response, auth}: HttpContextContract) {
    const groupId = await request.param('groupId') as number
  ...
  public async accept ({ request, response, bouncer }: HttpContextContract) {
    ...
    // No lucid sempre é necessário carregar as informações com o o "load"
    await groupRequest.load('group')
